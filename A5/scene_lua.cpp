...
extern "C"
int gr_noise_cmd(lua_State* L)
{
  float x = luaL_checknumber(L, 1);
  float y = luaL_checknumber(L, 2);
  int octaves = luaL_checknumber(L, 3);
  const siv::PerlinNoise::seed_type seed = 123456u;
	const siv::PerlinNoise perlin{ seed };
  luaL_newmetatable(L, "gr.noise");
  lua_setmetatable(L, -2);
  return int(floor(1000 * perlin.octave2D_01(x, y, octaves)));
}
// Create a Cone node
extern "C"
...
int gr_light_cmd(lua_State* L)
{
  ...
  int is_area = luaL_checknumber(L, 4);
  float light_size = luaL_checknumber(L, 5);
  l.is_area = is_area;
  l.light_size = light_size;
...
int gr_texture_cmd(lua_State* L)
{
  ...
  int x, y, n;
  unsigned char *img_data = stbi_load(filename, &x, &y, &n, 0);
  //stbi_set_flip_vertically_on_load(1);
  if (!img_data) {
    printf("Error loading texture\n");
    exit(1);
  }
  data->material = new TextureMaterial(img_data, x, y, n);
...
// Add a node's joint parent
extern "C"
int gr_node_add_parent_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  gr_node_ud* parentdata = (gr_node_ud*)luaL_checkudata(L, 2, "gr.node");
  luaL_argcheck(L, parentdata != 0, 2, "Node expected");

  SceneNode* parent = parentdata->node;
...
extern "C"
int gr_node_set_normals_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  GeometryNode* self = static_cast<GeometryNode*>(selfdata->node);
  ...
  self->normals = new TextureMaterial(img_data, x, y, n);
  return 0;
}
...
int gr_material_cmd(lua_State* L)
{
  ...
  double kd[3], ks[3], reflectivity[3];
  get_tuple(L, 1, kd, 3);
  get_tuple(L, 2, ks, 3);

  double shininess = luaL_checknumber(L, 3);
  get_tuple(L, 4, reflectivity, 3);
  bool dielectric = luaL_checknumber(L, 5);
  float refractive_index = luaL_checknumber(L, 6);
  int glossiness = luaL_checknumber(L, 7);
  data->material = new PhongMaterial(glm::vec3(kd[0], kd[1], kd[2]),
                                     glm::vec3(ks[0], ks[1], ks[2]),
                                     shininess, glm::vec3(reflectivity[0], reflectivity[1], reflectivity[2]), dielectric, refractive_index, glossiness);
  luaL_newmetatable(L, "gr.material");
  lua_setmetatable(L, -2);
...
static const luaL_Reg grlib_functions[] = {
  {"noise", gr_noise_cmd},
  {"node", gr_node_cmd},
  {"add_parent", gr_node_add_parent_cmd},
...
static const luaL_Reg grlib_node_methods[] = {
  ...
  {"set_normals", gr_node_set_normals_cmd},
