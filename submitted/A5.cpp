...
const int light_samples = 4; // how frequently to sample area lights for soft shadows (light_samples per each row and column)
const bool depth_of_field = false;
const float focal_length = 900.0f;
const float aperture = 75.0f;
const int dof_samples = 3;
const int recursion = 4; // recursion that controls number of times a ray can bounce(or refract) off a surface
const int row_samples = 4; //  how many samples to take per pixel for antialiasing( row_samples ^ 2)
