...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// The Reset Button slider
		if ( ImGui::Button( "Reset" ) ) {
			m_shape_color = vec3(1.0f, 1.0f, 1.0f);
	          	m_shape_translation = vec2(0.0f, 0.0f);
        		m_shape_size = 1.0f;
          		m_shape_rotation = 0.0f;
          		m_mouse_GL_coordinate = dvec2(0.0f);
          		m_mouseButtonActive = false;;
		}

		// Retrieve red color component from slider and store in the first element of
		// m_shape_color, similarly for the green and blue components.
		...
		// the rotation slider
		ImGui::SliderFloat("Rotation angle(Radians)", &m_shape_rotation, 0.0f, 2 * PI);
