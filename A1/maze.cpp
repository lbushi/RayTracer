...
glm::vec3 Maze::getStart() const {
	for (int i = 0; i < m_dim; ++i) {
		if (getValue(0, i) == 0) {
			return glm::vec3(0, 0, i);
		}
	}
	for (int i = 0; i < m_dim; ++i) {
		if (getValue(m_dim - 1, i) == 0) {
			return glm::vec3(m_dim - 1, 0, i);
		}
	}
	for (int i = 0; i < m_dim; ++i) {
		if (getValue(i, 0) == 0) {
			return glm::vec3(i, 0, 0);
		}
	}
	for (int i = 0; i < m_dim; ++i) {
		if (getValue(i, m_dim - 1) == 0) {
			return glm::vec3(i, 0, m_dim - 1);
		}
	}
}
